 ...
 * @author Claudemir da Luz <xmaxmex@gmail.com>
 * @version 1.3
 ...
 * Changes and Improvements by xmaxmex 07-2023
 * 
 * Create more functions Wrappers
 * Tests with Intelbras cameras Onvif models: 
 * vip-3230-B-SL-G2
 * vip-1020-B-G2
 * vip-1130-B-G2
 * vip-S330-G2 
...
class Ponvif {
	...
	public function getSnapshotUri() { return $this->snapshoturi; }

	public function getSystemDateAndTime() { return $this->core_GetSystemDateAndTime(); }
	public function getUriRTSP_Principal_Intelbras() { return $this->media_GetStreamUri("Profile000"); }
	public function getUriRTSP_Extra_Intelbras() { return $this->media_GetStreamUri("Profile001"); }
	...
	*/
	...
	public function getSnapShotAndSave($image_path_name_jpg) {

		$auth = $this->username.":".$this->password;

		$sources = $this->getSources();
		$ptoken = $sources[0][0]['profiletoken'];
		$url = $this->media_GetSnapshotUri($ptoken);

		// try Auth Basic
		$result = $this->getUrlAuthContent("$auth",$url);

		if ($result == false) {

			// try Auth Digest 
			$result = $this->getUrlAuthContent("$auth",$url,"1");
			if ($result == false) {
				return false;
			} else {
				file_put_contents($image_path_name_jpg, "$result");
				return true;
			}
	...
	*/
	...
	protected function getUrlAuthContent($auth,$url,$type="0"){
        $ch = curl_init();
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, 5); // maximum timeout to wait response
		if ($type == "1") {
			// Auth Digest
